...
# Install Automation {{{
COPY --chown=1000:1000 . /opt/fmc_repository/OpenMSA_Automation
RUN install_repo_deps.sh /opt/fmc_repository/OpenMSA_Automation/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/OpenMSA_Automation/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
